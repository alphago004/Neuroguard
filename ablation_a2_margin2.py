"""
Ablation A2: Margin m=2.0
Identical to main training but with contrastive loss margin changed from 3.0 to 2.0.
Run with: python3 ablation_a2_margin2.py
Results saved to: ablation_a2_results.txt
"""
...
DEVICE_CAP = 50
N_PAIRS    = 100_000
MARGIN     = 2.0   # ← the only change vs main training
...
def main():
    ...
    logger.info(f"=== ABLATION A2: MARGIN=2.0 === device={device}")
    ...
    with open(SAVE_DIR / "scaler_a2.pkl", "wb") as f:
    ...
    logger.info(f"A2 model (margin=2.0, full Transformer): {n_params:,} trainable parameters")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss - 1e-6:
            ...
            best_epoch       = epoch
            epochs_no_improv = 0
            torch.save(model.state_dict(), SAVE_DIR / "ablation_a2_margin2.pt")
            status = "saved"
        else:
            epochs_no_improv += 1
    ...
    a2_checkpoint = SAVE_DIR / "ablation_a2_margin2.pt"
    best_model, _ = build_model(transformer_layers=1, margin=MARGIN)
    best_model.load_state_dict(
        torch.load(a2_checkpoint, map_location=device)
    )
    ...
    logger.info("Running full evaluation on A2 checkpoint…")
    dna_map = enroll_all_devices(window_ds, scaler, checkpoint_path=a2_checkpoint)
    report  = evaluate_model(window_ds, dna_map, scaler, checkpoint_path=a2_checkpoint)
    ...
    logger.info("\n" + "=" * 55)
    logger.info("ABLATION A2 (MARGIN=2.0) COMPLETE")
    logger.info("=" * 55)
    ...
    with open(PROJECT_ROOT / "ablation_a2_results.txt", "w") as f:
        f.write("ABLATION A2 - MARGIN=2.0\n")
    ...
    logger.info("Results saved → ablation_a2_results.txt")
