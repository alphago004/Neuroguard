...
% =============================================================================
\begin{abstract}
The proliferation of IoT devices has dramatically expanded the attack surface
of modern networks, yet existing intrusion detection systems (IDS) remain
fundamentally limited by their dependence on known attack signatures.
We present \neuroguard{}, a zero-day IoT compromise detection system that
learns each device's behavioral identity exclusively from normal traffic ---
never from attack data --- and raises alerts when a device deviates from its
established fingerprint.

\neuroguard{} employs a Siamese neural network with shared encoder weights,
combining a three-layer feedforward architecture with a Transformer
self-attention block to capture both feature-level and temporal behavioral
patterns across 50-flow traffic windows.
Trained with contrastive loss on 539{,}612 candidate pairs drawn from 1{,}360
normal windows spanning 16 IoT devices, the encoder maps each
60-dimensional Zeek flow-feature vector to a 64-dimensional behavioral
embedding.
Device-specific alert thresholds are calibrated from enrollment baselines at
$k \in \{2.5,\,3.0\}$ standard deviations above each device's mean
embedding distance.

Evaluated on the TON\_IoT dataset~\cite{alsaedi2020toniot} under a strict
train-on-normal-only protocol --- attack data withheld from all training,
scaling, and enrollment phases --- \neuroguard{} achieves a
\textbf{ROC-AUC of 0.9402}, a \textbf{detection rate of 97.6--98.3\%}, and
a \textbf{false positive rate of 3.2--6.3\%} across 839 attack windows.
Per-type detection reaches \textbf{98.7\%} for backdoor and
\textbf{97.5\%} for ransomware attacks.
To complement per-window detection of acute threats, we introduce an
EWMA-based behavioral drift detector that identifies slow, sustained
compromise --- characteristic of APT activity --- within 15--25 observation
windows at a 50\% sustained elevation above the enrollment baseline, with
...
The Internet of Things (IoT) has grown to encompass an estimated 17 billion
connected devices worldwide, embedding networked computation into critical
infrastructure, healthcare, manufacturing, and consumer
environments~\cite{statista2023iot}.
This proliferation has dramatically expanded the network attack surface ---
and created a detection problem that existing IDS are structurally
ill-equipped to solve.

\subsection*{The Signature Problem}

Traditional network IDS --- whether rule-based (Snort, Suricata) or
anomaly-based (statistical profiling, flow whitelisting) --- fundamentally
depend on prior knowledge of attack patterns.
A new attack family generates no prior rule match and no known statistical
signature; this is not a parameter-tuning issue but an architectural
limitation.
The 2016 Mirai botnet demonstrated the consequence: hundreds of thousands of
IoT devices were compromised and weaponized before any signature
existed~\cite{antonakakis2017mirai}.
Advanced Persistent Threat (APT) actors exploit this gap deliberately,
operating below per-window detection thresholds for weeks before
launching final-stage payloads.

\subsection*{The Behavioral Identity Insight}

A network-connected device with a fixed function --- a thermostat, smart lock,
or industrial sensor --- exhibits remarkably stable traffic patterns during
normal operation.
It communicates with a predictable set of endpoints, at predictable intervals,
with predictable payload distributions.
This stability is an asset: a device's normal behavior can be fingerprinted
with high fidelity from a modest enrollment window, and any sustained
deviation from that fingerprint is a strong anomaly signal regardless of what
malware produced it.

\subsection*{Prior Work and Its Limitations}

Unsupervised anomaly detection methods --- IsolationForest~\cite{liu2008iforest},
OneClassSVM~\cite{scholkopf1999ocsvm}, and autoencoder-based
approaches~\cite{mirsky2018kitsune} --- have been applied to IoT traffic,
but operate on aggregate statistical features and lack per-device identity
modeling.
Kitsune~\cite{mirsky2018kitsune} achieves strong results on single-device
traces but does not generalize across heterogeneous device types without
retraining.
Federated approaches~\cite{nguyen2019diot} and flow-based ML
classifiers~\cite{moustafa2016unswnb15} require either attack labels or
centralized retraining.
No prior work combines contrastive identity learning with per-device
enrollment and EWMA drift detection in a zero-label setting.
...
\begin{enumerate}
  \item \textbf{Per-instance behavioral identity.}
    We demonstrate that Siamese contrastive learning builds per-device
    behavioral fingerprints from 50-flow windows alone, achieving an
    inter-to-intra class embedding separation ratio of $3.08\times$.
  ...
  \item \textbf{Transformer-augmented behavioral encoder.}
    ...
    shared encoder captures cross-feature behavioral correlations, contributing
    measurably to the separation ratio without increasing per-window latency.

  \item \textbf{Enrollment-calibrated scoring.}
    Device-specific alert thresholds calibrated from enrollment
    cosine-distance distributions adapt to per-device traffic variability,
    enabling \neuroguard{} to operate across both tight and variable IoT
    profiles without manual tuning.

  \item \textbf{EWMA behavioral drift for APT detection.}
    A lightweight per-device EWMA drift detector provides complementary
    coverage for slow, sustained compromise below the per-window alert
    threshold, firing within 15--25 windows of APT-style score elevation
    with zero additional false positives on normal traffic.
\end{enumerate}

Evaluated on TON\_IoT with a strict train-on-normal-only protocol,
\neuroguard{} achieves ROC-AUC 0.9402, detection rate 97.6--98.3\%, and
F1\,=\,0.9814 at $k=3.0$ --- without observing a single attack window during
any phase of model development.
...
The performance gap is categorical rather than incremental: all three baselines
achieve $\text{TPR} < 4\%$ with $\text{F1} < 0.06$, while \neuroguard{}
achieves $\text{TPR} = 97.6$--$98.3\%$ with $\text{F1} = 0.981$.
This represents a \textbf{32--51$\times$ improvement in detection rate} and
a \textbf{16--26$\times$ improvement in F1}.
...
zero-day threats.
The comparison against three state-of-the-art baselines trained on the same
data shows that the performance gap is categorical: all baselines achieve
$\text{TPR} < 4\%$ because they lack the per-device identity representation
that makes anomaly scores meaningful.
...
\textbf{Future work} falls along three axes.
\emph{Cross-dataset generalization:} evaluation on Bot-IoT~\cite{koroniotis2019botiot},
IoT-23~\cite{garcia2020iot23}, and Kitsune~\cite{mirsky2018kitsune} will test
whether behavioral embeddings transfer across network environments.
\emph{Enrollment efficiency:} few-shot or meta-learning approaches could
reduce the minimum enrollment requirement from 10 to 3--5 windows.
\emph{Multi-modal fusion:} fusing network flow embeddings with device telemetry
(temperature, motion, GPS readings present in TON\_IoT) could further tighten
enrollment baselines for constrained IoT profiles.

The behavioral identity framing --- ``is this device still behaving like
itself?'' --- is fundamentally complementary to signature-based and
anomaly-based IDS.
\neuroguard{} is designed to operate as a second-layer detector that is blind
to attack signatures by construction, making it resilient to the class of
threats that existing IDS are structurally incapable of detecting.
